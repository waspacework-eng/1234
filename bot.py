...
async def llm_free_search(uid: int, user_query: str) -> str:
    ...
    system_prompt = (
        ...
        f"База событий Москвы (ещё не показанные):\n{events_json}\n\n"
        f"Выбери до 5 подходящих. Верни ТОЛЬКО JSON:\n"
        f'{{"understood":"как понял запрос","events":[индексы из поля index, макс 5],"intro":"1-2 живых предложения","hooks":["почему подходит"]}}\n'
        f"Если ничего: {{\"understood\":\"...\",\"events\":[],\"intro\":\"\",\"hooks\":[]}}\n"
        f"Учитывай бюджет, время, настроение, предыдущий контекст диалога."
    )
